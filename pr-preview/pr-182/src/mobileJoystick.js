...
const circleInside = document.createElementNS('http://www.w3.org/2000/svg', 'circle');
...
mobileVelocity.createdJoystick = true
}
...
svg.addEventListener('touchmove', function(event) {
    ...
    let distanceToCenter = Math.sqrt(distanceFromCenterX * distanceFromCenterX + distanceFromCenterY * distanceFromCenterY)
    if(distanceToCenter < joystickRadius - joystickNippleRadius) {
        ...
        mobileVelocity.x = distanceFromCenterX / maxDistance
        mobileVelocity.y = distanceFromCenterY / maxDistance
    } else {
        const scaleFactor = maxDistance / distanceToCenter;
        distanceFromCenterX *= scaleFactor
        distanceFromCenterY *= scaleFactor
        distanceToCenter = Math.sqrt(distanceFromCenterX * distanceFromCenterX + distanceFromCenterY * distanceFromCenterY)
